...
7. Render the set with `metamorfiles_render_batch` using a single variant, for example `{ "template": "<id>", "name": "<id>-formats", "variants": [] }`, which renders the defaults in every format.
