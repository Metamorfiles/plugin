...
- Claude Code, Cowork, Desktop, Grok: `.claude-plugin/plugin.json` (with `userConfig.download_key`), `.claude-plugin/marketplace.json`, `.mcp.json` (passes the key as `METAMORFILES_KEY`)
...
- Gemini CLI: `gemini-extension.json` (`settings` entry with `envVar: METAMORFILES_KEY`, sensitive)
